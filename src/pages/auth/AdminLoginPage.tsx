...
import { Lock, ArrowLeft, Globe, Check } from 'lucide-react'
        ...
        <div className="max-w-7xl mx-auto flex h-14 items-center justify-between px-4">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-xs sm:text-sm font-extrabold tracking-wide">
              {isHi ? 'ऑल ड्राइवर्स कल्याण संगठन (सुपर एडमिन)' : 'All Drivers Welfare Association (Super Admin)'}
            </span>
          </div>
          <LangToggle />
        </div>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="text-center mb-8">
            <h1 className="text-2xl md:text-3xl font-extrabold text-blue-900 tracking-tight">
              {isHi ? 'सुपर एडमिन लॉगिन' : 'Super Admin Login'}
            </h1>
            <p className="mt-2 text-sm text-neutral-600 max-w-xs mx-auto leading-relaxed">
              {isHi ? 'जिले, भुगतान और सिस्टम सेटिंग्स प्रबंधित करने के लिए साइन इन करें' : 'Sign in to manage districts, payments, and system settings'}
            </p>
          </div>
          ...
          <motion.div {...popIn} className="bg-white rounded-2xl border border-neutral-200/80 shadow-xl shadow-neutral-100 p-6 md:p-8">
            <div className="mb-6 flex items-center gap-3.5 pb-5 border-b border-neutral-100">
              <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-orange-500/10 border border-orange-500/30 text-orange-600 shadow-sm">
                <Lock className="h-5 w-5" />
              </div>
              <div>
                <p className="font-extrabold text-neutral-900">{isHi ? 'सुपर एडमिन' : 'Super Admin'}</p>
                <p className="text-xs text-neutral-500">{isHi ? 'अपने ADWA एडमिन क्रेडेंशियल के साथ साइन इन करें' : 'Sign in with your ADWA admin credentials'}</p>
              </div>
            </div>
